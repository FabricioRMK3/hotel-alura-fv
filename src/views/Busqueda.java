...
public class Busqueda extends JFrame {
		...
		btnEliminar.addMouseListener(new MouseAdapter(){
			public void mouseClicked(MouseEvent e) {
				...
				if(filaReservas >= 0) {
					...
					huespedes = tbHuespedes.getValueAt(filaReservas, 0).toString();
					int confirmaH = JOptionPane.showConfirmDialog(null, "Desea borrar el huesped?");
		...
		try {
			for(Reserva reservas : reserva) {
				modelo.addRow(new Object[] {
						reservas.getId(), reservas.getDataE(), reservas.getDataS(), reservas.getValor(), reservas.getFormaPago()
						});
				}
			}catch(Exception e) {
				throw e;
		...
		try {
			for(Huespedes huespedes1 : huespedes) {
				modeloHuesped.addRow(new Object[] {
						huespedes1.getId(), huespedes1.getNombre(), huespedes1.getApellido(), huespedes1.getFechaNacimiento(), huespedes1.getNacionalidad(),huespedes1.getTelefono(), huespedes1.getIdReserva()
				});
			}
		}catch(Exception e) {
				throw e;
	...
	private void actualizarHuespedes() {
		...
		}, ()-> JOptionPane.showInternalMessageDialog(this, "aaa"));
